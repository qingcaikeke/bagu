...
**底层**：使用**内存屏障**的机制禁止重排序。+强制刷新cpu缓存实现可见性
...
内存泄漏：key(tLocal对象)是弱引用，val不是，这样的设计是因为，threadMap的生命周期是和thread一样的，不用的key不清理就会内存泄漏，使用弱引用使不用动key可以直接被回收，之后再次访问map，会清除key为null对应的val
...
抽象队列同步器，用于实现ReentrantLock，Semaphore等
