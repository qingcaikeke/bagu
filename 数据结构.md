...
128：最长连续序列：给个数组，找最长连续序列，可能有重。时间要求o(n)

​	第一反应先排序，然后遍历，如果下一个等于前一个+1，curLen+1；如果和当前相同，continue；

​	排序nlogn，想优化到on，想到空间换时间。原地哈希不行，数据范围可能远大于数组长度，

​		建个哈希set，遍历nums，如果num+1在set里，就暂时跳过他；如果不在，从num往前找，记录长度
...
忽略大小写和其他字符判断回文：toLowerCase+isLetterOrDigit
...
合并有序数组，结果放到nums1：双指针分别指向m-1,n-1，然后往前移动

#### 排序

56：合并区间int\[][2]：先按第一个排序，然后
...
倒数第k个节点：都从head出发，第一个先走k步，然后一二一起动，一到null，二为结果

19：删除倒数第k个节点：dummy出发，第一个先走k，fast.nxt = null终止，slow为要删的前一个

141：环形链表，都从head出发，一个步长为2，一个为1

142：检测入环位置，一二从head出发，相遇时三从head出发，二三相遇点为结果

61：旋转链表，找到倒数第k个的前一个位置，断链

​	因为k可能很大，所以要先求链表长度，因为已经有长度了，所以找倒数第k个不用双指针了
...
1.深度优先搜索，记录访问过的节点，节点被二次访问说明有环。

​	白灰黑三色节点，开始都是白，所有临界点访问完变黑，遍历过程中发现有一条边指向灰色节点，存在环

​	三色是因为不知道是回溯回来的还是碰到环了？

2.拓扑排序，计算节点入度，从入度为0的点开始入队，出队给邻接顶点入度减一

**判断无向图是否有环**

无向图没有入度出度的概念，不能用拓扑排序

1.深度优先搜索，靠visited+递归传入parent(上一个节点)，因为遍历周边节点会遍历到parent

​	不能三色标记，理由同上，遍历周边时回发先上一个是灰色，但实际上没环，因为是共用一条边

2.并查集：在遍历边 u - v 时，如果u的父亲和v的父亲是一个，那么存在环
